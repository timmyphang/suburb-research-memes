...
SYNTHESIS_SYSTEM = """You are a cultural researcher who synthesises web search
results into structured, lighthearted suburb profiles for Instagram scripts.
...
Rules:
- ALL content MUST be PG-13. No political, religious, or offensive material.
- Focus on low-stakes cultural quirks: coffee habits, parking complaints, dog breeds, fashion, weekend routines.
- If search results lack strong data for a section, be creative with gentle, universal suburban humor — but stay grounded.
- "citations" must reference real URLs found in the provided search results. Never invent URLs.
- "snippets_used" must list every search result that contributed information.
- The personas (ideal_resident, misfit) should be entertaining exaggerations suitable for a funny Instagram post.
- Target enough detail for a 400-word script, but keep each section concise."""
...
def process_suburb(prompt_data: dict) -> dict:
    ...
    try:
        ...
        # ── Step 1: Two targeted searches ──
        search_stereotype = pi_search(
            f'site:reddit.com/{reddit_sub} "{suburb}" stereotype OR "starter pack"',
            max_results=5,
        )
        time.sleep(0.5)  # gentle rate limit

        search_drama = pi_search(
            f'"{suburb}" funny local news OR council dispute OR local drama',
            max_results=5,
        )

        searches = {
            "stereotype": search_stereotype,
            "local_drama": search_drama,
        }

        n_results = (
            len(search_stereotype.get("results", []))
            + len(search_drama.get("results", []))
        )
        print(
            f"  [{suburb}] {n_results} search results "
            f"({len(search_stereotype.get('results', []))} stereotype, "
            f"{len(search_drama.get('results', []))} drama)",
            flush=True,
        )
